...
public class W7ImportDelegate extends EventDispatcher
{
         ...
         //   trace('--->\n'+windowXML.toXMLString())
            try {
              ...
              //  vo.name = windowXML.Name;
                ...
                if (!parseState.ok) {
                    throw new Error(parseState.errorMessage);
                }
    ...
    public function getShadingSystemManufacturer(vo:W7WindowImportVO, windowXML:XML):String{
        ...
        var glassLayerName:String;
        var glass:XML;
		if (vo.shadingSystemType=="WP"){ // "Window Panel"
			var allClear:Boolean = true;
			for each (glass in windowXML.GlzSysList.GlzSys.GlassList.Glass){
				glassLayerName = glass.Name;
        ...
        }else if (vo.shadingSystemType=="AF"){ // "Applied Film"
            for each (glass in windowXML.GlzSysList.GlzSys.GlassList.Glass){
                glassLayerName = glass.Name;
                if (glassLayerName.indexOf("CLEAR_")>-1){
                    continue;
    ...
    public function getShadingMaterialManufacturer(vo:W7WindowImportVO, windowXML:XML):String {
        ...
        //@todo fix me below "Variables of type XML cannot be undefined. The value undefined will be type coerced to XML before comparison."
        if (shadeMaterial==undefined){
            return "";
        }*/
        //GD changed the above to :
        const shadeMaterials:XMLList = windowXML.GlzSysList.GlzSys.ShadeList.ShadingLayer.ShadeMaterial;
        if (shadeMaterials.length() == 0) return "";
        const shadeMaterial:XML = shadeMaterials[0];
    ...
    private function wrapQuotesIfNeeded(str:String):String{
        if (str.indexOf(" ") != -1) {
            str = "\"" + str + "\""
        }
        return str;
    ...
    private function commandLineString(args:Vector.<String>, executable:File):String{
        var command:String = executable.nativePath;
        var parts:Array = [wrapQuotesIfNeeded(command)];
        var l:uint = args.length;
        for (var i:uint = 0; i<l ; i++){
            parts.push(wrapQuotesIfNeeded(args[i]))
        }
        return parts.join(" ");
    }
